...
export interface GlareCardProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onDrag' | 'onDragStart' | 'onDragEnd' | 'onAnimationStart'> {
...
const GlareCard = React.forwardRef<HTMLDivElement, GlareCardProps>(
  (
    ...
    // 1. Perspective Tilt (Dynamic 60fps GPU transform via useTransform)
    const rotateXDeg = useTransform(springY, (y) => y * -tiltIntensity);
    const rotateYDeg = useTransform(springX, (x) => x * tiltIntensity);
    const rotateX = useMotionTemplate`${rotateXDeg}deg`;
    const rotateY = useMotionTemplate`${rotateYDeg}deg`;
    ...
    // 4. Subtle Reflective Shimmer (Moves opposite to tilt)
    const shimmerOffset = useTransform(springX, (x) => `${x * -20}%`);
